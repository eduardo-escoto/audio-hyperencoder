...
def pre_encode_task(cfg: DictConfig) -> None:
    ...
    # Load model
    logger.info(f"Loading model: {pre_encode_cfg.model_name}")
    model = load_model(
    ...
    logger.info(f"Found {len(batches)} batches with {sum(len(files) for files in batches.values())} total files")
