...
class SchedulerConfig(BaseConfig):
    ...
    power: Optional[float] = Field(
        default=0.5,
        ge=0.0,
        description="Power parameter for InverseLR scheduler"
    )
    ...
    warmup: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Warmup ratio for scheduler"
    )
    ...
    gamma: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Gamma parameter for StepLR scheduler"
    )
...
class EncoderConfig(BaseConfig):
    ...
    c_mults: List[int] = Field(
        default=[16, 8, 4, 2, 2],
        description="Channel multipliers for each layer"
    )
    
    strides: List[int] = Field(
        default=[8, 8, 4, 4, 1],
        description="Stride values for each layer"
    )
    
    use_snake: bool = Field(
        default=False,
        description="Whether to use Snake activation"
    )
    ...
    def validate_equal_lengths(cls, v, info):
        ...
        if info.field_name == 'strides' and info.data.get('c_mults'):
            ...
            if len(v) != len(c_mults):
                raise ValueError(f"c_mults and strides must have the same length, got {len(c_mults)} and {len(v)}")
        return v
...
class DecoderConfig(BaseConfig):
    ...
    final_tanh: bool = Field(
        default=False,
        description="Whether to apply tanh activation at the end"
    )
...
class BottleneckConfig(BaseConfig):
    ...
    levels: Optional[List[int]] = Field(
        default=[8, 5, 5, 5],
        description="Quantization levels for FSQ bottleneck"
    )
    
    # VQ-specific parameters
    num_quantizers: Optional[int] = Field(
        default=None,
        ge=1,
        description="Number of quantizers for VQ bottleneck"
    )
    
    codebook_size: Optional[int] = Field(
        default=None,
        ge=1,
        description="Size of codebook for VQ bottleneck"
    )
    
    commitment_loss_weight: Optional[float] = Field(
        default=None,
        ge=0.0,
        description="Weight for commitment loss in VQ"
    )
...
class DemoConfig(BaseConfig):
    ...
    demo_every: int = Field(
        default=20,
        ge=1,
        description="Generate demo every N steps"
    )
    
    max_demos: int = Field(
        default=10,
        ge=1,
        description="Maximum number of demos to generate"
    )
    
    demo_length: Optional[int] = Field(
        default=None,
        ge=1,
        description="Length of demo sequences"
    )
    
    save_demos: bool = Field(
        default=True,
        description="Whether to save demo outputs"
    )
...
class ModelConfig(BaseConfig):
    ...
    encoder: EncoderConfig = Field(
        ...
        description="Encoder architecture configuration"
    )
    
    decoder: DecoderConfig = Field(
        default_factory=DecoderConfig,
        description="Decoder architecture configuration"
    )
    ...
    demo: Optional[DemoConfig] = Field(
        default_factory=DemoConfig,
        description="Demo and evaluation settings"
    )
        ...
        if info.field_name == 'decoder' and info.data.get('encoder'):
            ...
            if hasattr(encoder, 'latent_dim') and hasattr(v, 'latent_dim'):
                if encoder.latent_dim != v.latent_dim:
                    raise ValueError(f"Encoder and decoder latent_dim must match: {encoder.latent_dim} != {v.latent_dim}")
        return v
    ...
    def validate_latent_dim_consistency(cls, v, info):
        ...
        if info.data.get('encoder') and hasattr(info.data['encoder'], 'latent_dim'):
            if info.data['encoder'].latent_dim != v:
                raise ValueError(f"Model latent_dim must match encoder latent_dim: {v} != {info.data['encoder'].latent_dim}")
        return v 